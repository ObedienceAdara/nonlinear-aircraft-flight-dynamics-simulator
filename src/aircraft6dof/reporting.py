...
def _series(history, controls, environment, aircraft):
    ...
    for i, (ti, x) in enumerate(zip(t, X)):
        ...
        fp, mp = propulsion.force_and_moment(command_vec[3], V)
...
def export_simulation(history, output_dir: Path, controls, environment, aircraft):
    ...
    _plot(
        ...
        t,
        [
            np.rad2deg(a["aileron_command_rad"]), np.rad2deg(a["aileron_rad"]),
            np.rad2deg(a["elevator_command_rad"]), np.rad2deg(a["elevator_rad"]),
            np.rad2deg(a["rudder_command_rad"]), np.rad2deg(a["rudder_rad"]),
            100 * a["throttle"],
        ],
        ["Aileron command", "Aileron actual", "Elevator command", "Elevator actual", "Rudder command", "Rudder actual", "Throttle"],
        "Control Inputs", "Command / Actual (deg / %)",
    )
