...
def build_aircraft() -> AircraftModel:
    geometry = VehicleGeometry(mass_kg=1200.0, inertia_kg_m2=np.diag([1800.0, 2500.0, 4200.0]),
                               wing_area_m2=16.2, wing_span_m=10.9, mean_chord_m=1.49,
                               cg_to_ref_m=np.array([0.15, 0.0, 0.0]))
    aero = AeroCoefficients(CL0=0.25, CL_alpha=5.2, CL_de=0.55, CL_q=7.0,
    ...
    propulsion = Propulsion(max_thrust_N=4200.0, thrust_velocity_factor=0.25,
...
def initial_state() -> AircraftState:
    return AircraftState(position_ned_m=np.array([0.0, 0.0, -1000.0]), velocity_body_m_s=np.array([55.0, 0.0, 0.0]),
                          omega_body_rad_s=np.zeros(3), quaternion_bn=np.array([1.0, 0.0, 0.0, 0.0]))


def controls(t: float) -> ControlInput:
    return ControlInput(aileron=np.deg2rad(3.0 if 18.0 <= t < 26.0 else 0.0),
...
def environment(t: float) -> Environment:
    ...
    return Environment(wind_ned_m_s=np.array([5.0, 2.0, 0.0]), gust_ned_m_s=gust.value(t),
